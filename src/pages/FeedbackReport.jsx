...
function CompetencyCard({ comp, weight, onClick }) {
  ...
  return (
    <div className="fr-comp-card" onClick={onClick}>
...
function CompetencyModal({ comp, weight, onClose }) {
  if (!comp) return null;
  const color = competencyColor(comp.score ?? 0);
    ...
    <div className="fr-modal-overlay" onClick={onClose}>
      <div className="fr-modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="fr-modal-header">
          <div>
            <h3 className="fr-modal-title" style={{ color }}>
              {(comp.label || 'Competency').toUpperCase()}
            </h3>
            <div className="fr-score-simple" style={{ gap: '4px' }}>
              <span className="fr-score-number" style={{ fontSize: '32px', color }}>
                {comp.score ?? '—'}
              </span>
              <span className="fr-score-denom" style={{ fontSize: '12px' }}>/ 10</span>
            </div>
          </div>
        ...
        <div className="fr-modal-body">
          <p className="fr-modal-feedback">{comp.detailedFeedback || 'No detailed feedback available.'}</p>
          
          {comp.evidence && comp.evidence.length > 0 && (
            <div className="fr-modal-evidence" style={{ borderLeftColor: color }}>
              <span className="fr-modal-evidence-label" style={{ color }}>EVIDENCE</span>
              {comp.evidence.map((ev, i) => (
                <p key={i} className="fr-modal-evidence-text">"{ev}"</p>
              ))}
            </div>
          )}
        </div>
  ...
  return (
    <div className="fr-action-card">
      <div className="fr-action-content">
        <h3 className="fr-action-title">
          <span style={{ color: 'var(--color-gold)', marginRight: '12px' }}>{(index + 1).toString().padStart(2, '0')}</span>
          {action.title}
        </h3>
        <p className="fr-action-desc">{action.description}</p>
        
        {(action.whyItMatters || action.howToPractice) && (
          <div className="fr-action-meta-grid">
            {action.whyItMatters && (
              <div>
                <span className="fr-action-sublabel">WHY IT MATTERS</span>
                <p className="fr-action-meta-text">{action.whyItMatters}</p>
              </div>
            )}
            ...
            {action.howToPractice && (
              <div>
                ...
                <p className="fr-action-meta-text">{action.howToPractice}</p>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div className="fr-root">
      ...
      <main className="fr-main">
        {/* ── HERO DASHBOARD ───────────────────────────────────── */}
        <section className="fr-hero-dashboard">
          <div className="fr-hero-left">
            <div className="fr-score-simple">
              <span className="fr-score-number" style={{ color: gradeColor }}>
                {overallScore}
              </span>
          ...
          <div className="fr-hero-right">
            <div className="fr-hero-meta">
              <span className="fr-meta-highlight">{mode.replace(/_/g, ' ')}</span>
            ...
            {feedback.executiveSummary ? (
              <p className="fr-summary">{feedback.executiveSummary}</p>
            ) : (
              <p className="fr-summary" style={{ color: 'var(--color-text-muted)' }}>
        ...
        {feedback.priorityActions?.length > 0 && (
          <section className="fr-section">
            <div className="fr-section-header">
              ...
              <p className="fr-section-subtitle">Focus on these targeted improvements before your next session.</p>
            </div>
        ...
        {feedback.transcript && (
          <section className="fr-section">
            <div className="fr-section-header">
              ...
              <p className="fr-section-subtitle">Full timestamped speech-to-text recording.</p>
            </div>
            <div className="fr-transcript-box">
              <p className="fr-transcript-text">
                {feedback.transcript}
              </p>
            </div>
          </section>
